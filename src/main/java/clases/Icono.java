...
public class Icono extends JGObject {
    ...
    public Icono(String nombre, double x, double y, String graf, short id, short tipoIcono, int cantidad,
            short pertenece, String nombreLogico, Habilidad habilidad) {
        // tipo icono :
        // 0 = habilidades (habilidad)
        // 1 = inventario (objeto)
        super(nombre, true, x, y, (int) Math.pow(2, 4), graf);
        this.graphicName = graf;
        this.setIdObjeto(id);
    ...
    public Icono(String nombre, double x, double y, String graf, short id, short tipoIcono, int cantidad,
            short pertenece, String nombreLogico, Objeto item) {
        // tipo icono :
        // 0 = habilidades (habilidad)
        // 1 = inventario (objeto)
        super(nombre, true, x, y, (int) Math.pow(2, 4), graf);
     ...
     * @param pertenece : Referencia para el caso comerciar donde se muestra los
     *                  item del Jugador y Npc
     * @param item      : Objeto que representa al item respectivo con todos sus
     *                  datos.
     */
    ...
    public void paint() {
        ...
        if (graphicName != null) {
            eng.drawImage(x, y, graphicName);
        }

        // Hover Effect
        double mx = eng.getMouseX() + eng.viewXOfs();
        double my = eng.getMouseY() + eng.viewYOfs();

        // Assuming approx 32x32 size for icons (centered)
        if (mx >= x - 18 && mx <= x + 18 && my >= y - 18 && my <= y + 18) {
            eng.setColor(JGColor.white);
            eng.drawRect(x - 19, y - 19, 38, 38, false, false, 2.0, JGColor.white); // Highlight box
        }
    }
